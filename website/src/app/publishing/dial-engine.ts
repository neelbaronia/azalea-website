...
export class Datamosh {
  ...
  private measure() {
    ...
    this.edges = [];
    for (let index = 0; index <= this.settings.cols; index += 1) {
      this.edges.push(
        Math.round(
          this.width *
            Math.pow(index / this.settings.cols, this.settings.perspective),
        ),
      );
    }
  }
  ...
  private liveFlow(column = 0) {
    ...
    const finalColumn = Math.max(1, this.settings.cols - 1);
    ...
    const localProgress =
      (phase - CYCLE_HOLD - columnLag) / movementDuration;
  ...
  private draw() {
    ...
    for (let column = 0; column < this.settings.cols; column += 1) {
      ...
      for (let tile = this.settings.tiles + 2; tile >= -2; tile -= 1) {
        ...
        const bottom =
          Math.round(this.tileEdge(distance + 1) * height) + bleed;
  ...
  setResolve(value: number) {
    ...
    if (next > 0 && this.resolve === 0) this.targetFlow = this.chooseTargetFlow();
